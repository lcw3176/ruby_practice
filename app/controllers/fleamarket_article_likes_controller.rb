class FleamarketArticleLikesController < ApplicationController
  ...
  def create
    ...
    likes = FleamarketArticleLike.new(user_id: params[:user_id], fleamarket_article_id: params[:article_id])
    likes.save
  ...
  def destroy
    ...
    exist_like_check = FleamarketArticleLike.find_by(user_id: user_id, fleamarket_article_id: article_id)

    if exist_like_check.nil?
      return render json: response_format(code: "no_exist", message: "좋아요 한 이력이 없어요"), status: :not_found
    end

    FleamarketArticleLike.destroy_by(user_id: user_id, fleamarket_article_id: article_id)
